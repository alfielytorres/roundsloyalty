              ...
              <tr key={d.id} className="hover:bg-black/[0.02] transition-colors group">
                ...
                <td className="px-5 py-4">
                  <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    {d.status === 'active' && (
                      <form action="/api/devices/pause" method="POST">
                    ...
                    {d.status === 'paused' && (
                      <form action="/api/devices/resume" method="POST">
                        <input type="hidden" name="device_id" value={d.id} />
                        <button type="submit" className="text-xs font-semibold text-black/70 hover:text-black px-3 py-1.5 rounded-xl hover:bg-black/5 transition-colors">
                          Resume
                        </button>
                      </form>
                    )}
                    {d.status !== 'revoked' && (
                      <form action="/api/devices/revoke" method="POST">
                        <input type="hidden" name="device_id" value={d.id} />
          ...
          <div key={d.id} className="px-5 py-4">
            ...
            <div className="flex items-center gap-2 mt-3">
              ...
              {d.status === 'paused' && (
                <form action="/api/devices/resume" method="POST">
                  <input type="hidden" name="device_id" value={d.id} />
                  <button type="submit" className="text-xs font-semibold text-black/70 px-3 py-1.5 rounded-xl border border-black/10 hover:bg-black/5 transition-colors">
                    Resume
                  </button>
                </form>
              )}
    ...
    <main className="px-5 pt-10 pb-32">
      <div className="max-w-4xl mx-auto">
        <div className="flex items-start justify-between mb-7">
          ...
          <div className="mt-1">
            <RegisterDeviceModal vendorId={vendor.id} />
          </div>
        </div>
