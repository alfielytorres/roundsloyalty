...
struct VendorSettingsView: View {
                    ...
                    List {
                        Section(header: Text("Store Details").foregroundColor(.brandTaupe)) {
                            VStack(alignment: .leading, spacing: 6) {
                                Text("Business Name").font(.caption.weight(.semibold)).foregroundColor(.brandDarkGreen)
                                TextField("Coffee House etc.", text: $businessName)
                            }
                            VStack(alignment: .leading, spacing: 6) {
                                Text("Description").font(.caption.weight(.semibold)).foregroundColor(.brandDarkGreen)
                                TextField("Speciality coffee roasters...", text: $businessDescription)
                            }
                            VStack(alignment: .leading, spacing: 6) {
                                Text("Address").font(.caption.weight(.semibold)).foregroundColor(.brandDarkGreen)
                        ...
                        Section(header: Text("Loyalty Program").foregroundColor(.brandTaupe)) {
                            ...
                            if programType == .stamp {
                                ...
                                VStack(alignment: .leading, spacing: 6) {
                                    Text("Reward").font(.caption.weight(.semibold)).foregroundColor(.brandDarkGreen)
                                ...
                                VStack(alignment: .leading, spacing: 6) {
                                    Text("Reward description").font(.caption.weight(.semibold)).foregroundColor(.brandDarkGreen)
                                    TextField("e.g. 10 points = free drink", text: $rewardDescription)
                                }
                            } else {
                                Text("Tiered programs require the web portal to configure tiers.")
                                    .font(.caption).foregroundColor(.brandTaupe)
                            }
                        }
                        .listRowBackground(Color.white.opacity(0.7))
                        ...
                        if let successMessage {
                            Section {
                                Text(successMessage).font(.caption).foregroundColor(.brandGreen)
                            }
                            .listRowBackground(Color.white.opacity(0.7))
                        }
                        ...
                        Section {
                            Button { Task { await saveSettings() } } label: {
                                HStack {
                                    ...
                                    else { Text(existingBusiness == nil ? "Create Store" : "Save Changes").fontWeight(.semibold) }
                                    Spacer()
                                }
                                .padding(.vertical, 4)
                            }
                        ...
                        Section {
                            Button(role: .destructive) { Task { await signOut() } } label: {
                                HStack {
                                    Spacer()
                                    if isSigningOut { ProgressView().tint(.red) }
                                    else { Label("Sign Out", systemImage: "arrow.right.square") }
                                    Spacer()
                                }
                            }
                        }
    ...
    private func loadSettings() async {
        ...
        do {
            let biz: Business? = try await supabase
                .from("businesses").select().eq("owner_id", value: userId).maybeSingle().execute().value
            ...
            if let biz {
                ...
                let program: LoyaltyProgram? = try await supabase
                    ...
                    .eq("business_id", value: biz.id).eq("is_active", value: true)
        ...
        } catch { print("Failed to load settings: \(error)") }
        isLoading = false
    }
    ...
    private func buildConfig() -> [String: Any] {
        switch programType {
        case .stamp: return ["stamps_required": stampsRequired, "reward": rewardDescription]
        case .points: return ["points_per_visit": pointsPerVisit]
        case .tiered: return [:]
        }
    }
