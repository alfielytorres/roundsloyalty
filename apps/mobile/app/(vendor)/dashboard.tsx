...
interface LedgerEntry {
  ...
  delta: number
  created_at: string
  customer_vendor_memberships: { profiles: { display_name: string | null } }
}
...
export default function VendorDashboard() {
  ...
  const { data: stats, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const [membersRes, weekLedgerRes] = await Promise.all([
        supabase.from('customer_vendor_memberships').select('id', { count: 'exact', head: true }).eq('vendor_id', vendor!.id).eq('status', 'active'),
        supabase.from('loyalty_ledger').select('id', { count: 'exact', head: true }).eq('vendor_id', vendor!.id).gte('created_at', weekAgo),
      ])
      return { totalMembers: membersRes.count ?? 0, activityThisWeek: weekLedgerRes.count ?? 0 }
    },
    enabled: !!vendor,
  })
  ...
  const { data: recentActivity } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase
        .from('loyalty_ledger')
        .select(`id, event_type, delta, created_at, customer_vendor_memberships (profiles (display_name))`)
  ...
  if (!vendor) {
    return (
      <View style={styles.noVendorContainer}>
        ...
        <Text style={styles.noVendorText}>Complete your store profile to start rewarding your customers.</Text>
        <TouchableOpacity style={styles.setupButton} onPress={() => router.push('/(vendor)/settings')}>
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.inner}>
      ...
      <TouchableOpacity style={styles.scanButton} onPress={() => router.push('/(vendor)/qr')}>
        ...
        recentActivity.map((entry) => {
          ...
          return (
            <View key={entry.id} style={styles.activityRow}>
              ...
              <Text style={styles.activityText}><Text style={{ fontWeight: '700' }}>{name}</Text>{' — '}{label}</Text>
...
const styles = StyleSheet.create({
  ...
  noVendorContainer: { flex: 1, justifyContent: 'center', alignItems: 'center', backgroundColor: Colors.background, padding: 40 },
  ...
  setupButton: { backgroundColor: Colors.primary, borderRadius: 16, paddingVertical: 14, paddingHorizontal: 32 },
  ...
  statCard: { flex: 1, backgroundColor: Colors.surface, borderRadius: 20, padding: 16, borderWidth: 1, borderColor: Colors.border },
  ...
  scanButton: { backgroundColor: Colors.primary, borderRadius: 20, paddingVertical: 16, alignItems: 'center', marginBottom: 32 },
