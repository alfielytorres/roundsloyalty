...
interface Membership {
  ...
  loyalty_balances: Array<{ stamps_collected: number; points_accumulated: number; total_visits: number; last_active_at: string | null }>
}
  ...
  const { data: memberships, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        ...
        .select(`id, membership_token, status, joined_at, vendors (id, business_name, category), loyalty_balances (stamps_collected, points_accumulated, total_visits, last_active_at)`)
  ...
  if (!memberships?.length) {
    return (
      <View style={styles.empty}>
        ...
        <Text style={styles.emptyText}>Show the My QR tab at a participating store to earn stamps and rewards.</Text>
      </View>
    )
  }
    ...
    <View style={styles.container}>
      ...
      <FlatList
        ...
        renderItem={({ item }) => {
          ...
          return (
            <TouchableOpacity style={[styles.card, isPending && styles.cardPending]} onPress={() => router.push(`/(customer)/cards/${item.id}`)} activeOpacity={0.85}>
              <View style={styles.cardHeader}>
                <View style={styles.logoPlaceholder}><Text style={styles.logoInitial}>{item.vendors.business_name.charAt(0).toUpperCase()}</Text></View>
                <View style={{ flex: 1 }}>
                  <Text style={styles.businessName}>{item.vendors.business_name}</Text>
                  <Text style={styles.meta}>{isPending ? 'Show your QR to activate' : `${visits} visit${visits !== 1 ? 's' : ''}`}</Text>
                </View>
...
const styles = StyleSheet.create({
  ...
  logoPlaceholder: { width: 44, height: 44, borderRadius: 14, backgroundColor: Colors.primaryLight, justifyContent: 'center', alignItems: 'center' },
  ...
  pendingBadge: { backgroundColor: 'rgba(251,191,36,0.15)', borderRadius: 10, paddingHorizontal: 10, paddingVertical: 4 },
  pendingBadgeText: { fontSize: 11, fontWeight: '700', color: Colors.warning },
  stampRow: { flexDirection: 'row', alignItems: 'center', gap: 6 },
  stamp: { width: 18, height: 18, borderRadius: 9, borderWidth: 1.5, borderColor: Colors.stampEmpty, backgroundColor: 'transparent' },
