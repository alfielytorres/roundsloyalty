-- Fix request_reward_collection(uuid, text):
-- 1. Also look for collection_requested instances (not just 'available')
--    so a customer can't create a second one while one is pending
-- 2. Deduct rounds_required from current_rounds when creating on-the-fly instance
...
BEGIN
  ...
  IF v_reward_id IS NULL THEN
    -- No existing instance — check if customer has enough rounds to create one
